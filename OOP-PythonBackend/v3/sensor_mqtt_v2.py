...
class Sensor_MQTT:
    ...
    def __processMessage(self, client, userdata, msg):
        ...
        if "temp" in msg.topic:
            ...
            if self.__messageDate == "0":
                self.__messageDate = datetime.now(timezone.utc).strftime(
                    "%Y-%m-%d"
                )
            else:
            ...
            if self.__messageTime == "0":
                self.__messageTime = datetime.now(timezone.utc).strftime(
                    "%H:%M:%S"
                )
            else:
