/*
    Embedded program of group Viziduc WireBark 2020 - Final 2nd. sem project
        ...
        Check readme.txt before working with this program!
...
#include <CircularBuffer.h>                         // FIFO queue library
...
#define RXPinGPS 4                                  // <-----
#define TXPinGPS 5                                  // <-----
#define DEEP_SLEEP 300e6                            // in microseconds
...
void mqttPublish(void) {
	while (!light_stor.isEmpty()) {
		...
		time_a[strlen(time_a)-2] = '\0';            // Removes centi-seconds
		...
		delay(100);                                 // Delay in order to not spam Python
	}
}
...
void setup() {
	...
	if(light < light_treshold) {
		...
		wifi_set_sleep_type(LIGHT_SLEEP_T);         // allowing entry to light sleep during idle
		...
		digitalWrite(IN_LED, HIGH);                 // reversed (HIGH == led off)

		mqttClient.setServer(MQTT_SERVER, MQTT_PORT);

		// first measurement and fail-check
...
void loop() {
	if (light >= light_treshold && maintenanceMode == false) {
		...
		gpsParser();                                // Read GPS data
		if (maintenanceMode == false) {
			if (latitude == 0.0 && longitude == 0.0 && gpsRetries < 60) {
				...
				if (gpsCooldown + gpsRetries < 20) {
					...
					gpsRetries = 0;
					gpsCooldown = 0;
					mqttClient.publish(ROOT_TOPIC, "published, sleeping");
					delay(100);
					sleep();
				}
			}
		...
		sensorACK();
		mqttClient.publish(ROOT_TOPIC, "it's getting dark");
		delay(100);
		sleep();
	}
}
